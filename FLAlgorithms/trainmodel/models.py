...
class Net(nn.Module):
    ...
    def forward(self, x, start_layer_idx = 0, logit=False, start_layer_output=False):
        """
        :param x:
        ...
        if start_layer_output:    
            layer_ = self.layers[start_layer_idx]
            layer_ = layer_(x)   
        ...
        if logit:
            restults['logit'] = z
        if start_layer_output:
            restults['strat_layer_output'] = layer_
        return restults
