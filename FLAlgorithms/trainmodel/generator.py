...
class Generator(nn.Module):
    ...
    def forward(self, labels, Real_CL_Results, latent_layer_idx=-1, verbose=True):
        """
        G(Z|y) or G(X|y):
        ...
        #print(z.size())
        #z = z * Real_CL_Resultsw 

        z = torch.cat((z, Real_CL_Results), dim=1)
...
class Discriminator(nn.Module):
    def __init__(self, size=0,dataset='mnist', model='cnn', embedding=False):
    ...
    def forward(self, img,cl_score):
        img = self.model_1(img).squeeze(-1)
        cl_score = self.model_2(cl_score)
        return img, cl_score
